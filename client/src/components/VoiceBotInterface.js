...
const VoiceBotInterface = ({ botType = 'business' }) => {
  ...
  const startSession = async () => {
    try {
      ...
      const tokenResponse = await fetch(`http://localhost:8000/token?voice=coral&bot_type=${botType}`);
  ...
  return (
    <div className="voice-bot-container" style={{ borderTop: `4px solid ${currentBot.color}` }}>
      ...
      <div className="voice-bot-controls">
        {!isSessionActive ? (
          ...
          <div className="active-controls">
            <button 
              className={`control-btn microphone ${isListening ? 'active' : ''}`}
